...
**AC: what in the world is happening here?** 
...
for (u in urls){
  ...
  release_text <- release_text |> 
    add_row(url = u, text = str_squish(text))
}
...
**Answer**
The function treats "Dog" and "dog" as separate words due to the capitalization differences. 
...
**Answer**
The function unnest_tokens created the column "word" where all the unique words in the text are identified; that said, the function is also point to "d" as a word, for example. 
...
**Answer**
It is showing the count of unique words in a bar chart. It isn't intersting because it is only picking up the generic words, like "the." 


**AC Questions: what is "reorder(word, n))" doing?**
**what is xlab(NULL) doing?**
...
Well, that's a bit underwhelming - a lot of very common (and short) words. 

This also is a solved problem in working with text data, and words like "a" and "the" are known as "stop words". 

In most cases you'll want to remove them from your analysis since they are so common. Tidytext provides a dataframe of them that we'll load, and then we'll add some of our own.
...
**Answer**
Yes, it is more interesting. Health is the number one word. I don't know anything about him but this would make me want to look into his platforms to see whether health issues are on there, or perhaps something big in maryland has been happening related to health. It also interesting that Baltimore makes the list, as it is the only placed-based word in the top 50. 
...
2021 contains words pertaining to the pandemic, funding (probably associated with the pandemic) and legislation (also probably associated with the pandemic).

2022 contains Baltimore along with more variations of "community" along with support and health. 

2023 contains "women" and "fire" which is interesting to me. I cannot recall anything that would explain why "women" would make the top 10. Perhaps all the wild fires in 2023 is why fire makes the cut? The Canadian fires, while a big deal, didn't last terribly long, so I would be surprised if they had that many press releases about it. 
...
**AC Question** what does unnest the tokens mean? What does break apart summary mean? 
...
**Answer**
It seems like placed-based projects were a priority, and likely related to covid-19. Human rights and the Ches Bay make another appearance as well. 
...
**Answer**
Oral health stands out. Very specific. Human rights is still there, along with the Chesapeake Bay. Seems to be more of a focus on human rights related legislation, supreme court, etc. Covid-19 is not longer in the top. 
...
Critical seems incorrect. "Critical" legislation during the pandemic, for example, which necessary and while perhaps not positive due to circumstance, also not negative. Critical thinking is also positive, as another example. 

Risk is also another subjective word. 

I would also be curious on whether a blind person would consider "blind" as a negative word. 

Shady is another one. Shady environments in urban settings is a good thing. A shady politician ... not so much. 

Competitive could also go either way depending on context. 
